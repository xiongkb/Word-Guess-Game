...
var music = new Audio(
  "assets/images-sound/melodyloops-preview-be-happy-1m50s.mp3"
);
...
function beginGame() {
  ...
  for (var i = 0; i < randomWord.length; i++) {
    if (randomWord.charAt(i) === " ") {
      //solving the space so there is no _ for a space, but a space
      charSpace += "&nbsp;";
    } else {
      charSpace += "_&nbsp;";
    }
    ...
    if (
      !unique.includes(randomWord.charAt(i)) &&
      randomWord.charAt(i) !== " "
    ) {
  ...
  document.onkeyup = function (keypressed) {
    ...
    if (alphabet.includes(userInput)) {
      //chances left
      if (!unique.includes(userInput) && !guessed.includes(userInput)) {
        // loses code
        chances -= 1;
        chancesLeft.innerHTML = chances;
      }
      ...
      // this nested if else will check if you already use the letter
        ...
        if (unique.includes(userInput)) {
          //checking if user letter matches the selected word letter
          ...
          for (var i = 0; i < unique.length; i++) {
            for (var j = 0; j < pickedWord.length; j++) {
              if (pickedWord.charAt(j) === unique[i]) {
                pickedWord =
                  pickedWord.substr(0, j) + "_" + pickedWord.substr(j + 1);
              }
            }
          }
...
function stop() {
  ...
  //     "<h3 class='box'>Thanks for playing</h3><br>" +
  //     "Won: " +
  //     wins +
  //     "<br>" +
  //     "Lost: " +
